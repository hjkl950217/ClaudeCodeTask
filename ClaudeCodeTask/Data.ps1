...
# 性能关键：行级处理用 Add-Type 内联 C#（PowerShell 逐行调用开销是 C# 的 24 倍，实测 17s → 0.7s）
...
# 内联 C# 扫描器（进程内只编译一次；类名带版本号——签名变更必须换名，见上方注释）
...
public static class CctScannerV4 {
    ...
    // firstCwd = 会话启动/存储目录（jsonl 物理存放目录的编码依据，第九轮分组键）；
    ...
    // 第十一轮：并行全扫。文件间无共享状态，Parallel.For 并发读盘+扫描；单文件失败
