...
- 增量缓存：热启动扫描加速 ~65-73%
  *Incremental cache: warm-start scanning is ~65-73% faster.*
- `cct clear` 清理：无参清 cct 扫描缓存（下次扫描全量重建）；`-cc` 按行数判定清理多余 Claude Code 会话（每目录保留最新 ≥ 阈值会话，先预览过滤规则与待删清单、确认后删除）
  *`cct clear` cleanup: bare call clears the cct scan cache (fully rebuilt on next scan); `-cc` removes surplus Claude Code sessions by line-count threshold (keeps the newest ≥ threshold per folder, previews the filter rules and delete list, then deletes after confirmation).*
...
- [Pester 6](https://pester.dev)（仅开发测试需要 *required for dev testing only*）

### 方式一：从 PowerShell Gallery 安装（推荐）Option 1: Install from PowerShell Gallery (recommended)

```powershell
# 安装模块（命令为 cct）Install the module (the command is cct)
...
```powershell
# 更新到最新版 Update to the latest version
...
### 方式二：从代码仓库安装（体验源码 / 开发调试）Option 2: Install from source (try the code / development)
...
### 中国大陆网络访问说明 Access from mainland China
...
**1. 配置代理 1. Configure a proxy**（推荐，最通用 *recommended, most universal*）

PowerShell 的 NuGet 提供程序遵循系统代理设置，设置后即可正常访问：
*PowerShell's NuGet provider follows the system proxy settings:*

```powershell
# 临时设置（当前终端生效）Temporary settings (current terminal only)
...
**2. 离线安装 2. Offline install**（无代理时的兜底 *fallback when no proxy*）
...
```powershell
cct                    # 弹出任务列表，空白输入 = 全部  Show task list; blank input = all
cct <关键词>           # 带初始过滤词  With an initial filter keyword
cct clear              # 清理 cct 扫描缓存（cache.json），下次扫描全量重建  Clear the cct scan cache (cache.json); fully rebuilt on next scan
cct clear -cc          # 清理多余 Claude Code 会话（先预览过滤规则与待删清单，确认后删除）  Clean surplus Claude Code sessions (previews filter rules and delete list, deletes after confirmation)
```

`cct clear` 选项 Options：
...
### 手动加载本地运行（临时终端，不改 profile）Manual local run (temporary terminal, no profile changes)
...
```
├── .github/           # GitHub 集成（CI 测试 + PSGallery 发布工作流 + issue 模板）GitHub integration (CI tests + PSGallery release workflow + issue templates)
...
- [Claude Code](https://github.com/anthropics/claude-code) —— Anthropic 官方命令行工具 *Anthropic's official CLI*
- [LINUX DO](https://linux.do) —— 新的理想型技术社区 *a new ideal tech community*
